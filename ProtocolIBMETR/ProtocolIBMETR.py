...
class ProtocolIBMETR:
	...
	def TKGen(self:object, idR:Element) -> tuple: # $\textbf{TKGen}(\textit{id}_R) \rightarrow \textit{dk}_{\textit{id}_R}$
		# Check #
		if not self.__flag:
	...
	def Enc(self:object, ekidS:Element, idRev:Element, message:int|bytes) -> Element: # $\textbf{Enc}(\textit{ek}_{\textit{id}_S}, \textit{id}_\textit{Rev}, m) \rightarrow \textit{ct}$
		...
		if isinstance(message, int): # type check
			...
			m = int.from_bytes(b"ProtocolIBMETR", byteorder = "big") & self.__operand
			print("Enc: The message passed should be an integer or a ``bytes`` object but it is not, which has been defaulted to b\"ProtocolIBMETR\". ")
	...
	def Dec(self:object, dkidR:tuple, idRev:Element, idSnd:Element, cipher:tuple) -> bytes: # $\textbf{Dec}(\textit{dk}_{\textit{id}_R}, \textit{id}_\textit{Rev}, \textit{id}_\textit{Snd}, \textit{ct}) \rightarrow m$
		...
		if isinstance(cipher, tuple) and len(cipher) == 6 and isinstance(cipher[0], int) and all([isinstance(ele, Element) for ele in cipher[1:]]): # hybrid check
			...
			ct = self.Enc(self.__group.random(ZR), self.__group.random(ZR), b"ProtocolIBMETR")
	...
	def TVerify(self:object, tkidR:tuple, cipher:tuple) -> bool: # $\textbf{TVerify}(\textit{tk}_{\textit{id}_R}, \textit{ct}) \rightarrow y, y \in \{0, 1\}$
		...
		if isinstance(cipher, tuple) and len(cipher) == 6 and isinstance(cipher[0], int) and all([isinstance(ele, Element) for ele in cipher[1:]]): # hybrid check
			...
			ct = self.Enc(self.__group.random(ZR), self.__group.random(ZR), b"ProtocolIBMETR")
			print("TVerify: The variable $\\textit{ct}$ should be a tuple containing an integer and 5 elements but it is not, which has been generated with $m$ set to b\"ProtocolIBMETR\". ")
...
def Protocol(curveType:tuple|list|str, round:int = None) -> list:
	...
	# Initialization #
	protocolIBMETR = ProtocolIBMETR(group)
	timeRecords, memoryRecords = [], []
	
	# Setup #
	startTime = time()
	mpk, msk = protocolIBMETR.Setup()
	endTime = time()
	...
	ek_id_S = protocolIBMETR.EKGen(id_S)
	endTime = time()
	timeRecords.append(endTime - startTime)
	memoryRecords.append(process.memory_info().rss)
	...
	dk_id_R = protocolIBMETR.DKGen(id_R)
	endTime = time()
	timeRecords.append(endTime - startTime)
	memoryRecords.append(process.memory_info().rss)
	...
	# TKGen #
	startTime = time()
	tk_id_R = protocolIBMETR.TKGen(id_R)
	endTime = time()
	timeRecords.append(endTime - startTime)
	memoryRecords.append(process.memory_info().rss)
	...
	# Enc #
	startTime = time()
	message = int.from_bytes(b"ProtocolIBMETR", byteorder = "big")
	ct = protocolIBMETR.Enc(ek_id_S, id_R, message)
	endTime = time()
	timeRecords.append(endTime - startTime)
	memoryRecords.append(process.memory_info().rss)
	...
	# Dec #
	startTime = time()
	m = protocolIBMETR.Dec(dk_id_R, id_R, id_S, ct)
	endTime = time()
	timeRecords.append(endTime - startTime)
	memoryRecords.append(process.memory_info().rss)
	...
	# TVerify #
	startTime = time()
	bRet = protocolIBMETR.TVerify(tk_id_R, ct)
	endTime = time()
	timeRecords.append(endTime - startTime)
	memoryRecords.append(process.memory_info().rss)

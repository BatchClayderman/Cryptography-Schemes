...
class ProtocolHIBME:
	...
	def Enc(self:object, ekIDS:tuple, IDSnd:tuple, IDRev:tuple, message:bytes|int) -> Element: # $\textbf{Enc}(\textit{ek}_{\textit{ID}_S}, \textit{ID}_\textit{Rev}, M) \rightarrow \textit{CT}$
		...
		if isinstance(message, int): # type check
			...
			M = int.from_bytes(b"ProtocolHIBME", byteorder = "big") & self.__operand
			print("Enc: The message passed should be an integer or a ``bytes`` object but it is not, which has been defaulted to b\"ProtocolHIBME\". ")
	...
	def Dec(self:object, dkIDR:tuple, IDRev:tuple, IDSnd:tuple, cipher:tuple) -> bytes: # $\textbf{Dec}(\textit{dk}_{\textit{ID}_R}, \textit{ID}_\textit{Rev}, \textit{ID}_\textit{Snd}, \textit{CT}) \rightarrow M$
		...
		if isinstance(cipher, tuple) and len(cipher) == 5 and isinstance(cipher[0], int) and all([isinstance(ele, Element) for ele in cipher[1:]]): # hybrid check
			...
			print("Dec: The variable $\\textit{CT}$ should be a tuple containing an integer and 4 elements but it is not, which has been generated with $M$ set to b\"ProtocolHIBME\". ")
...
def Protocol(curveType:tuple|list|str, l:int, m:int, n:int, round:int = None) -> list:
	# Begin #
	...
	# Enc #
	startTime = time()
	message = int.from_bytes(b"ProtocolHIBME", byteorder = "big")
	...
	# Dec #
	startTime = time()
	M = protocolHIBME.Dec(dk_ID_R, ID_Rev, ID_Snd, CT)
	MDerived = protocolHIBME.Dec(dk_ID_RDerived, ID_Rev, ID_Snd, CT)
	endTime = time()
	timeRecords.append(endTime - startTime)
	memoryRecords.append(process.memory_info().rss)
...
def main() -> int:
	...
	columns = [																						\
		"curveType", "secparam", "l", "m", "n", "isSystemValid", "isDeriverPassed", "isProtocolCorrect", 				\
			...
			for l in (5, 10, 15, 20, 25, 30):
				for m in range(5, l, 5):
					for n in range(5, l, 5):
						...
						if rounds:
							length, average = len(columns), [rounds[0][0], rounds[0][1], l, m, n]

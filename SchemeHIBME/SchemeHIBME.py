...
class Parser:
	def __init__(self:object, arguments:tuple|list) -> object:
		...
		self.__schemeName = "SchemeHIBME" # os.path.splitext(os.path.basename(__file__))[0]
...
def conductScheme(curveType:tuple|list|str, l:int = 30, m:int = 20, n:int = 10, round:int|None = None) -> list:
	# Begin #
...
def main() -> int:
			...
			for l in range(10, 31, 5):
				for m in range(5, l, 5):
					for n in range(5, l, 5):
						average = conductScheme(curveType, l = l, m = m, n = n, round = 0)
						for round in range(1, roundCount):
							result = conductScheme(curveType, l = l, m = m, n = n, round = round)
	...
	# End #
	errorLevel = EXIT_SUCCESS if results and all(all(tuple(r == roundCount for r in result[6:9]) + tuple(r > 0 for r in result[9:length])) for result in results) else EXIT_FAILURE
	try:

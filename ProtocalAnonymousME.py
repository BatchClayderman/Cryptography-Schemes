...
class ProtocalAnonymousME:
	...
	def DerivedKGen(self:object, _mpk:tuple, skIDkMinus1:tuple, IDk:tuple) -> tuple: # DerivedKGen(mpk, sk_ID_kMinus1, ID_k) -> sk_ID_k
		...
		if (
			...
			if self.__flag[0]:
				mpk, msk = self.Setup(l = self.__l)
				print("At least one of the parameters passed does not match the variable $l = {0}$. The program will call the ``Setup``, ``KGen``, and ``DerivedKGen`` with all the necessary variables generated in sequence. ".format(self.__l))
			else:
				mpk, msk = self.Setup()
				print("The ``Setup`` procedure has not been called yet. The program will call the ``Setup``, ``KGen``, and ``DerivedKGen`` with all the necessary variables generated in sequence. ")
			ID_k = tuple(self.__group.random(ZR) for i in range(self.__l - 1))
	...
	def Dec(self:object, cipher:tuple, skIDk:tuple) -> bytes: # Dec(CT, sk_ID_k) -> M
		...
		if self.__flag[0]:
			if isinstance(skIDk, tuple) and len(skIDk) >= 9 and isinstance(cipher, tuple) and 4 == len(cipher):
				...
				print("At least one of the variables $\\textit{CT}$ and $\\textit{sk}_\\textit{ID}_k$ is invalid. The program will call the ``Setup``, ``KGen``, ``Enc``, and ``Dec`` with all the necessary variables generated in sequence. ")
		else:
			mpk, msk = self.Setup()
			ID_k = tuple(self.__group.random(ZR) for i in range(self.__l - 1))
			M = self.randomElement()
			sk_ID_k = self.KGen(mpk, msk, ID_k)
			CT = self.Enc(mpk, ID_k, M)
			print("The ``Setup`` procedure has not been called yet. The program will call the ``Setup``, ``KGen``, ``Enc``, and ``Dec`` with all the necessary variables generated in sequence. ")
...
def protocal(curveType:str, k:int, l:int) -> list:
	...
	# Initialization #
	protocalAnonymousME = ProtocalAnonymousME(group)
	timeRecords, memoryRecords = [], []
	
	# Setup #
	startTime = time()
	mpk, msk = protocalAnonymousME.Setup(l)
	endTime = time()
	...
	startTime = time()
	sk_ID_k = protocalAnonymousME.KGen(mpk, msk, ID_k)
	endTime = time()
	timeRecords.append(endTime - startTime)
	memoryRecords.append(process.memory_info().rss)
	
	# DerivedKGen #
	sk_ID_kMinus1 = protocalAnonymousME.KGen(mpk, msk, ID_k[:-1]) # remove the last one to generate the sk_ID_kMinus1
	startTime = time()
	sk_ID_kDerived = protocalAnonymousME.DerivedKGen(mpk, sk_ID_kMinus1, ID_k)
	endTime = time()
	timeRecords.append(endTime - startTime)
	memoryRecords.append(process.memory_info().rss)
	
	# Enc #
	message = protocalAnonymousME.randomElement(GT)
	startTime = time()
	CT = protocalAnonymousME.Enc(mpk, ID_k, message)
	endTime = time()
	timeRecords.append(endTime - startTime)
	memoryRecords.append(process.memory_info().rss)
	...
	# Dec #
	startTime = time()
	M = protocalAnonymousME.Dec(CT, sk_ID_k)
	endTime = time()
	timeRecords.append(endTime - startTime)
	memoryRecords.append(process.memory_info().rss)
	
	# End #
	del protocalAnonymousME
...
def main() -> int:
	results, fileName = [], "ProtocalAnonymousME.xlsx"
